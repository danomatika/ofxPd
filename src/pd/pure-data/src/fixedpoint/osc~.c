...
static void *osc_new(t_floatarg f)
{
    ...
    x->x_f = ftofix(f);
	//post("osc [%x] new: input %f stored as %i/%f at %x", x, f, x->x_f, fixtof(x->x_f), &(x->x_f) );
...
static t_int *osc_perform(t_int *w)
{
    ...
    int n = (int)(w[4]);
    t_sample *tab = fixed_cos_table;
    unsigned int phase = x->x_phase;
    int conv = x->x_conv;
    int off;
    int frac;
	
	//printf("osc [%x] perform: stored %i/%f, curr %f [%x], phase %i\n", x, x->x_f, fixtof(x->x_f), fixtof(*in), in, x->x_phase );
